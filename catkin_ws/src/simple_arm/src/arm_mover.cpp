...
// function to check and clamp the joint angles to a safe zone
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2)
{
        ...
        // get min and max joint paramters, and assign them to their respective variables
        ...
        node2.getParam(node_name + "/min_joint_1_angle", min_j1);
        node2.getParam(node_name + "/max_joint_1_angle", max_j1);
        node2.getParam(node_name + "/min_joint_2_angle", min_j2);
        node2.getParam(node_name + "/max_joint_2_angle", max_j2);

        // check if joint 1 is in the safe zone, otherwise clamp it
        ...
        // check if joint 2 is in the safe zone, otherwise clamp it
                ...
                simple_arm::GoToPosition::Response& res)
{
        ...
        // check if requested joint angles are in the safe zone
...
{
        // initialize the arm_mover node and create and handle to it
